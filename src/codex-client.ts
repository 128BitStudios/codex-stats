...
export class CodexAPIClient {
  ...
  constructor(authData: AuthData) {
    ...
    this.model = this.getConfiguredModel()
  }
  ...
  private getConfiguredModel(): string {
    ...
    const fallbackModel = this.getDefaultModel(codexHome)
    ...
    try {
      ...
      return modelMatch?.[1] || fallbackModel
    } catch (error) {
      console.error('Error reading Codex config file:', error)
      return fallbackModel
    }
  }

  private getDefaultModel(codexHome: string): string {
    ...
    try {
      ...
      const cacheContent = fs.readFileSync(modelsCachePath, 'utf8')
      const modelsCache = JSON.parse(cacheContent)
      const cachedDefaultModel = modelsCache.models?.[0]?.slug
      return typeof cachedDefaultModel === 'string'
        ? cachedDefaultModel
        : 'gpt-5.5'
    } catch (error) {
      console.error('Error reading Codex models cache file:', error)
      return 'gpt-5.5'
    }
  }
}
